 ...
 * @version $Id: $Id
 */
@CommandTag(command = "year", overwrite = true)
public class GetYearCommand implements CommandLogic<Integer> {
  /**
   * {@inheritDoc}
   */
  @Override
  ...
  public Integer execute(List<Object> parameters) {
    return ObjectUtils.isEmpty(parameters) ?
        DateHelper.getYear() :
        DateHelper.getYear(Long.parseLong(String.valueOf(parameters.get(0))));
  }
}
