...
/**
 * <p>AllNotContainsTextValidation class.</p>
 *
 * @author yunaz.ramadhan on 7/22/2024
 * @version $Id: $Id
 */
@ValidationTag(command = "ALL_NOT_CONTAINS_TEXT", alias = {"all not contains text", "all has no text"}, overwrite = true)
public class AllNotContainsTextValidation extends ValidatorLogic<List<String>, String> {
  ...
  public boolean validate() throws Exception {
    ...
    if (getActual() == null || getActual().isEmpty()) {
      ...
      setReason(String.format("%s of %s items not contains expected text", matched, getActual().size()));
    }
