 ...
 * @version $Id: $Id
 */
@ValidationTag(command = "IS_LOWERCASE", alias = {"is lowercase"}, overwrite = true)
public class IsLowerCaseValidation extends ValidatorLogic<String, Boolean> {
  ...
  protected String setDefaultMessage() {
    return "Data still not in lower case";
  }
  ...
  public boolean validate() throws Exception {
    ...
    if (CommonHelper.isBlank(getActual()) || CommonHelper.isBlank(getExpected())) {
      ...
      boolean actual = getActual().equals(getActual().toLowerCase());
