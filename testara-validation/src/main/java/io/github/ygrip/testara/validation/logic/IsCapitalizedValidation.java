 ...
 * @version $Id: $Id
 */
@ValidationTag(command = "IS_CAPITALIZED", alias = {"is capitalized"}, overwrite = true)
public class IsCapitalizedValidation extends ValidatorLogic<String, Boolean> {
  ...
  protected String setDefaultMessage() {
    return "Data still not capitalized";
  }
  ...
  public boolean validate() throws Exception {
    ...
    if (CommonHelper.isBlank(getActual()) || CommonHelper.isBlank(getExpected())) {
      ...
      boolean actual = getActual().equals(StringHelper.capitalize(getActual()));
