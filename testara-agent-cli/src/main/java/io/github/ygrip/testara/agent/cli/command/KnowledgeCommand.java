...
@Command(name = "knowledge",
    ...
    subcommands = {
        KnowledgeCommand.StatusCommand.class,
        KnowledgeCommand.RefreshCommand.class,
...
public class KnowledgeCommand implements Runnable {
  ...
  static class RefreshCommand implements Runnable {
    ...
    public void run() {
      ...
      store.refresh(root);
      var snapshot = store.loadOrIndex(root);
      System.out.println("Knowledge refreshed: " + snapshot.stats().trackedFiles()
          + " files, " + snapshot.stats().scenarioCount() + " scenarios");
    }
  }
