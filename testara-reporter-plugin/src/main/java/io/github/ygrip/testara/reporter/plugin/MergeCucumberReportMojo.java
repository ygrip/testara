 ...
 */
@Mojo(name = "merge-cucumber", defaultPhase = LifecyclePhase.POST_INTEGRATION_TEST)
public class MergeCucumberReportMojo extends AbstractMojo {
  private final Logger log = Logger.getLogger(MergeCucumberReportMojo.class.getName());
   ...
   * @required
   */
  @Parameter(property = "report-name", defaultValue = "cucumber.json", required = true)
    ...
    if (!skip) {
      ...
      try {
        CucumberSummaryReportGenerator.fromLocation(path)
            .mergeReportAs(report);
      } catch (Exception e) {
        e.printStackTrace();
        throw new MojoExecutionException("Fail to merge cucumber json report");
      }
    } else {
