 ...
 */
@Log4j2
public final class AppiumInteractionCapability extends AppiumElementResolver implements InteractionCapability {
  private final AppiumDriver driver;

  public AppiumInteractionCapability(AppiumDriver driver) {
    this.driver = driver;
  }
  ...
  public InteractionCapability focus(Element locator) {
    WebElement target = element(locator);
    if (ObjectUtils.isNotEmpty(target)) {
      executeScript("arguments[0].focus();", target);
    }
    return this;
  }
  ...
  public InteractionCapability blur(Element locator) {
    WebElement target = element(locator);
    if (ObjectUtils.isNotEmpty(target)) {
      executeScript("arguments[0].blur();", target);
    }
    return this;
  }
  ...
  public TextEntry enter(String text) {
    return locator -> {
      ...
      return AppiumInteractionCapability.this;
    };
  }
