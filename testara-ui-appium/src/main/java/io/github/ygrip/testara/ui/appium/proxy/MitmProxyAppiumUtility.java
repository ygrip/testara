 ...
 * routing device traffic through the mitmproxy instance.
 */
@TestComponent(scope = RegistryScope.TEST)
@Log4j2
public class MitmProxyAppiumUtility extends AbstractProxy<Proxy> {
  ...
  public void start() {
    if (!CommonHelper.isBlank(getProxy())) {
      log.debug("#MitmProxy Appium utility already started");
      return;
    }
    try {
      log.debug("#Starting MitmProxy Appium utility via API at {}", mitmApiUrl);
      ...
      log.info("#MitmProxy Appium proxy started at {}", proxyAddr);
    } catch (Exception e) {
      log.error("Failed to start MitmProxy Appium utility", e);
    }
  }
  ...
  public void stop() {
    if (CommonHelper.isBlank(getProxy())) {
      log.info("#No active MitmProxy Appium proxy, ignoring stop");
      return;
    }
    try {
      if (instanceId != null) {
