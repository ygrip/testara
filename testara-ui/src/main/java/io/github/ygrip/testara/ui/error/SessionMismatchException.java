package io.github.ygrip.testara.ui.error;

/**
 * <p>SessionMismatchException class.</p>
 *
 * @author yunaz.ramadhan on 12/23/2019
 * @version $Id: $Id
 */
public class SessionMismatchException extends RuntimeException {
  /**
   * <p>Constructor for SessionMismatchException.</p>
   *
   * @param errorMessage a {@link String} object.
   */
  public SessionMismatchException(String errorMessage) {
    super(errorMessage);
  }

  /**
   * <p>Constructor for SessionMismatchException.</p>
   *
   * @param errorMessage a {@link String} object.
   * @param err a {@link Throwable} object.
   */
  public SessionMismatchException(String errorMessage, Throwable err) {
    super(errorMessage, err);
  }
}
